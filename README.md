...
- 👥 **在线成员**：实时显示谁在房间里。
- 🔔 **消息通知**：未读数显示在标题和标签页图标上，新消息有提示音；页面在后台 / 失焦时弹系统通知，点击跳回对应房间；可按房间静音。
- 📱 **可加到主屏（PWA）**：手机 / 电脑都能"添加到主屏幕 / 安装"，独立窗口打开，带团子图标；离线也能打开外壳。
- 🖥️ **macOS 桌面版**：Electron 壳，双击即用、无需终端；启动时选「自建服务器」或「加入服务器」，见 [desktop/](desktop/)。
...
public/
  ...
  manifest.webmanifest / sw.js / icons/   PWA 清单、离线 Service Worker 与图标
  vendor/            内置的 sjcl 与 qrcode-generator（随源码提交）
test/server.test.js  端到端测试
scripts/gen-icons.mjs   零依赖生成 PWA 图标
desktop/             macOS 桌面壳（Electron，复用 server.js；核心服务端仍零依赖）
docs/
